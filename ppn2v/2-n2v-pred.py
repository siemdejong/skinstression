...
from ppn2v.pn2v import prediction, utils
...
print(
    "Avg PSNR Prior:",
    np.mean(np.array(meanRes)),
    "+-(2SEM)",
    2 * np.std(np.array(meanRes)) / np.sqrt(float(len(meanRes))),
)
print(
    "Avg PSNR MMSE:",
    np.mean(np.array(results)),
    "+-(2SEM)",
    2 * np.std(np.array(results)) / np.sqrt(float(len(results))),
)
